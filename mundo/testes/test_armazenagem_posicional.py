...
def test_receber_carga_em_transito_gera_operacao_invalida():
    app = criar_app(com_loop_real_time=False)
    with TestClient(app) as cliente:
        motor = instancia_do_mundo.obter_motor()
        motor.energia.alocar_energia("reserva_estrategica", "armazenagem", 100)
        motor.cargas["c1"] = CargaMineral(
            "c1", "hematita", 10.0, 100.0, local=LocalDaCarga.EM_TRANSITO
        )
        ...
        eventos = motor.eventos.consultar_eventos()
        assert any(e.tipo == "operacao_invalida" and "trânsito" in e.dados["motivo"] for e in eventos)
        assert motor.armazens["armazem-1"].pilha == []
        assert motor.cargas["c1"].local == LocalDaCarga.EM_TRANSITO
